...
module.exports = (req, res, next) => {
    ...
    logger.info(`${process.env.WORKER_ID} Access [${ip},${country}] path:`, req.path, 'query:', JSON.stringify(req.query), 'params:', JSON.stringify(req.params), 'body:', JSON.stringify(req.body), `reqId:${req.reqId}`);
    ...
    try {
        ...
        global.pgdb_slave.query(qr, (err, result) => {
            if (err) {
        ...
        logger.error('Cookies or DB problem', e && e.message, e && e.stack, `reqId:${req.reqId}`);
        next();
    }
}
